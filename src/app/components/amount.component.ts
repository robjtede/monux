...
export class AmountComponent {
  ...
  @Input('show-currency') readonly showCurrency?: boolean = true
  @Input('large-major') readonly largeMajor?: boolean = false

  get showNative() {
    return !this.show || this.show === 'both' || this.show === 'native'
  }

  get showLocal() {
    return (
      this.amount.foreign &&
      (!this.show || this.show === 'both' || this.show === 'local')
    )
  }
}

export type SignModes = 'always' | 'onlyPositive' | 'onlyNegative' | 'never'
export type ShowModes = 'both' | 'local' | 'native'
